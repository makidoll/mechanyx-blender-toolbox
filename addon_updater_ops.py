	...
	def execute(self, context):
		...
		if not settings:
			updater.print_verbose(
			    "Could not get {} preferences, update check skipped".
			    format(__package__)
			)
			return {'CANCELLED'}
...
class AddonUpdaterInstallManually(bpy.types.Operator):
	...
	def draw(self, context):
		...
		if updater.update_link is not None:
			...
			row.operator(
			    "wm.url_open", text="(failed to retrieve direct download)"
			)
			row.enabled = False
...
def updater_run_install_popup_handler(scene):
	...
	elif "version_text" in updater.json and updater.json["version_text"
	                                                    ].get("version"):
...
def update_notice_box_ui(self, context):
	...
	if not updater.manual_only:
		...
		col.operator(
		    AddonUpdaterInstallManually.bl_idname, text="Install manually"
		)
	else:
		# ops = col.operator("wm.url_open", text="Direct download")
		# ops.url=updater.update_link
...
def update_settings_ui(self, context, element=None):
	...
	if updater.error is not None:
		...
		if "ssl" in updater.error_msg.lower():
			split.enabled = True
			split.operator(
			    AddonUpdaterInstallManually.bl_idname, text=updater.error
			)
		else:
			split.enabled = False
		...
		split.scale_y = 2
		split.operator(
		    AddonUpdaterCheckNow.bl_idname, text="", icon="FILE_REFRESH"
		)
		...
		split.scale_y = 2
		split.operator(
		    AddonUpdaterCheckNow.bl_idname, text="Addon is up to date"
		)
		split = sub_col.split(align=True)
		split.scale_y = 2
		split.operator(
...
# -----------------------------------------------------------------------------
classes = (
    AddonUpdaterInstallPopup, AddonUpdaterCheckNow, AddonUpdaterUpdateNow,
    AddonUpdaterUpdateTarget, AddonUpdaterInstallManually,
    AddonUpdaterUpdatedSuccessful, AddonUpdaterRestoreBackup,
    AddonUpdaterIgnore, AddonUpdaterEndBackground
)
